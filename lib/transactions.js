...
/* Propogates a provided Transaction Hex. */
function Propogate(options, callback) {
  if ((options.hex || options.txHex) && options.base) {
    var hex;
    if (options.hex) hex = options.hex;
    else hex = options.txHex;
    var options = {
      url: options.base + 'tx/push',
      method: 'POST',
      form: {'hex': hex}
    }
